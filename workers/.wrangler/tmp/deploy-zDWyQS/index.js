...
// src/plan-config.ts
var PLAN_CONFIG = {
  free: { dmPerMonth: 1e3, maxCampaigns: 1, platformFeePercent: 8, allowD2C: false, pricePerMonth: 0 },
  standard: { dmPerMonth: 1e4, maxCampaigns: 3, platformFeePercent: 8, allowD2C: false, pricePerMonth: 9900 },
  growth: { dmPerMonth: 5e4, maxCampaigns: 10, platformFeePercent: 7, allowD2C: false, pricePerMonth: 49e3 },
  pro: { dmPerMonth: -1, maxCampaigns: -1, platformFeePercent: 6, allowD2C: true, pricePerMonth: 99e3 }
};
...
__name(handleWebhookVerification, "handleWebhookVerification");
        ...
        if (entry.changes?.length) {
          ...
          for (const change of entry.changes) {
            if (change.field === "messages" && change.value) {
              await processMessagingEvent(change.value, brandIgId, supabase);
            } else if (change.field === "comments" && change.value) {
              await handleCommentEvent(change.value, brandIgId, supabase);
            }
          }
        }
      }
...
async function processMessagingEvent(event, brandIgId, supabase) {
  ...
  if (mid) {
    const { data: existing } = await supabase.from("share2dm_dm_queue").select("id, status").eq("mid", mid).maybeSingle();
    if (existing) {
      console.log("[DM] Duplicate mid, skip:", mid, "status:", existing.status);
      return;
    }
  }
  const { data: brand } = await supabase.from("share2dm_brands").select("*").eq("ig_account_id", brandIgId).single();
  console.log("[DM] brand lookup brandIgId:", brandIgId, "\u2192", brand?.id ?? "NOT FOUND");
  if (!brand) return;
  const { data: campaign } = await supabase.from("share2dm_campaigns").select("*").eq("ig_contents_id", reelVideoId).eq("brand_id", brand.id).eq("is_active", true).single();
  console.log("[DM] campaign lookup reelVideoId:", reelVideoId, "\u2192", campaign?.id ?? "NOT FOUND");
  if (!campaign) return;
  const planLimits = PLAN_CONFIG[brand.plan];
  if (planLimits && planLimits.dmPerMonth !== -1) {
    const startOfMonth = /* @__PURE__ */ new Date();
    startOfMonth.setUTCDate(1);
    startOfMonth.setUTCHours(0, 0, 0, 0);
    const { count } = await supabase.from("share2dm_dm_logs").select("*", { count: "exact", head: true }).eq("brand_id", brand.id).gte("dm_sent_at", startOfMonth.toISOString());
...
${trackingUrl}`;
  ...
  const { data: claimed } = await supabase.from("share2dm_dm_queue").update({ status: "sending" }).eq("campaign_id", campaign.id).eq("sender_ig_id", senderId).eq("status", "pending").select("id").maybeSingle();
  if (!claimed) {
  ...
  try {
    ...
    if (response.ok) {
      await supabase.from("share2dm_dm_queue").update({ status: "sent", sent_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", claimed.id);
      ...
      await supabase.from("share2dm_dm_queue").update({ status: "pending" }).eq("id", claimed.id);
...
__name(processMessagingEvent, "processMessagingEvent");
async function handleCommentEvent(value, brandIgId, supabase) {
  ...
  const { data: existingComment } = await supabase.from("share2dm_comment_logs").select("id").eq("comment_id", commentId).maybeSingle();
  if (existingComment) {
  ...
  if (!brand) return;
  const { data: campaign } = await supabase.from("share2dm_campaigns").select("*").eq("ig_contents_id", mediaId).eq("brand_id", brand.id).eq("campaign_type", "comment_automation").eq("is_active", true).maybeSingle();
  console.log("[Comment] campaign lookup mediaId:", mediaId, "\u2192", campaign?.id ?? "NOT FOUND");
  if (!campaign) return;
  const keywords = campaign.trigger_keywords ?? [];
  const lowerText = commentText.toLowerCase();
  const hasKeyword = keywords.length === 0 || keywords.some((kw) => lowerText.includes(kw.toLowerCase()));
  if (!hasKeyword) {
  ...
  if (campaign.comment_reply_message) {
    try {
      ...
      if (replyRes.ok) {
        commentRepliedAt = (/* @__PURE__ */ new Date()).toISOString();
...
${trackingUrl}`;
  ...
  const { data: claimed } = await supabase.from("share2dm_dm_queue").update({ status: "sending" }).eq("campaign_id", campaign.id).eq("sender_ig_id", commenterIgId).eq("status", "pending").select("id").maybeSingle();
  if (!claimed) {
...
__name(handleCommentEvent, "handleCommentEvent");
async function computeSignature(body, appSecret) {
  const encoder = new TextEncoder();
...
async function handleTracking(request, env) {
  ...
  const supabase = createClient(env.SUPABASE_URL, env.SUPABASE_SERVICE_KEY);
  await supabase.from("share2dm_dm_logs").update({ link_clicked_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("campaign_id", campaignId).eq("sender_ig_id", senderIgId);
  const { data: campaign } = await supabase.from("share2dm_campaigns").select("product_url").eq("id", campaignId).single();
  if (!campaign) {
    return new Response("Not found", { status: 404 });
  }
  const productUrl = campaign.product_url.startsWith("http") ? campaign.product_url : `https://${campaign.product_url}`;
  return Response.redirect(productUrl, 302);
}
__name(handleTracking, "handleTracking");

// src/auth.ts
async function processOAuth(code, redirectUri, env) {
  ...
  if (!pagesData.data?.length) {
    ...
    if (bizData.data?.length) {
      for (const biz of bizData.data) {
        ...
        if (bizPagesData.data?.length) {
          pagesData.data = bizPagesData.data;
          break;
        }
      }
    }
  ...
  const supabase = createClient(env.SUPABASE_URL, env.SUPABASE_SERVICE_KEY);
  const expiresAt2 = longTokenData.expires_in ? new Date(Date.now() + longTokenData.expires_in * 1e3).toISOString() : null;
  const { data: brand, error } = await supabase.from("share2dm_brands").upsert(
    {
...
__name(processOAuth, "processOAuth");
async function handleOAuthCallbackGet(request, env) {
  const url = new URL(request.url);
...
__name(handleOAuthCallbackGet, "handleOAuthCallbackGet");
...
async function handleClozetCallback(request, env) {
  const url = new URL(request.url);
  const token = url.searchParams.get("token");
  const brandId = url.searchParams.get("brand_id");
  const state = url.searchParams.get("state") ?? "";
  const origin = url.searchParams.get("origin") ? decodeURIComponent(url.searchParams.get("origin")) : "https://share2dm.xyz";
  const dashboardUrl = `${origin}/dashboard`;
  if (!token || !brandId) {
    return Response.redirect(`${dashboardUrl}?clozet_error=missing_params`, 302);
  }
  const supabase = createClient(env.SUPABASE_URL, env.SUPABASE_SERVICE_KEY);
  const { data: tokenRow, error: tokenErr } = await supabase.from("share2dm_connect_tokens").select("token, seller_profile_id, share2dm_brand_id, expires_at, used").eq("token", token).single();
  ...
  const { data: sellerProfile } = await supabase.from("seller_profile").select("id, store_name").eq("id", tokenRow.seller_profile_id).single();
  if (!sellerProfile) {
    return Response.redirect(`${dashboardUrl}?clozet_error=seller_not_found`, 302);
  }
  const { error: updateErr } = await supabase.from("share2dm_brands").update({
    clozet_seller_id: sellerProfile.id,
    clozet_store_name: sellerProfile.store_name,
    clozet_connected_at: (/* @__PURE__ */ new Date()).toISOString()
  }).eq("id", brandId);
  if (updateErr) {
  ...
  await supabase.from("share2dm_connect_tokens").update({ used: true }).eq("token", token);
...
__name(handleClozetCallback, "handleClozetCallback");
async function handleClozetContentLookup(url, env) {
  const brandId = url.searchParams.get("brand_id");
  ...
  const supabase = createClient(env.SUPABASE_URL, env.SUPABASE_SERVICE_KEY);
  const { data: brand } = await supabase.from("share2dm_brands").select("clozet_seller_id, clozet_store_name").eq("id", brandId).single();
  ...
  const { data: content, error } = await supabase.from("contents").select("id, short_code, ig_contents_code, store_name").eq("short_code", igCode).single();
  if (error || !content) {
...
__name(handleClozetContentLookup, "handleClozetContentLookup");
...
async function approveBilling(env, billingKey, customerKey, orderId, orderName, amount) {
...
__name(approveBilling, "approveBilling");
function getNextBillingDate(fromDate) {
  const next = new Date(fromDate);
  next.setMonth(next.getMonth() + 1);
  return next.toISOString().split("T")[0];
}
__name(getNextBillingDate, "getNextBillingDate");
async function savePaymentLog(supabase, brandId, paymentKey, orderId, plan, amount, tossResponse) {
...
__name(savePaymentLog, "savePaymentLog");
async function handleIssueBillingKey(request, env) {
  ...
  if (updateErr) {
    ...
    return new Response(
      ...
      { status: 500, headers: CORS_HEADERS2 }
    );
  }
  ...
  return new Response(
    ...
    { status: 200, headers: CORS_HEADERS2 }
  );
}
__name(handleIssueBillingKey, "handleIssueBillingKey");
async function handleBillingCron(env) {
  const supabase = createClient(env.SUPABASE_URL, env.SUPABASE_SERVICE_KEY);
  const today = (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
  const { data: brands, error } = await supabase.from("share2dm_brands").select("id, plan, toss_billing_key, toss_customer_key, next_billing_date").eq("next_billing_date", today).not("toss_billing_key", "is", null).neq("plan", "free");
  if (error) {
  ...
  for (const brand of brands) {
    ...
    if (result.ok) {
      const nextBillingDate = getNextBillingDate(/* @__PURE__ */ new Date());
      await supabase.from("share2dm_brands").update({
        next_billing_date: nextBillingDate,
...
__name(handleBillingCron, "handleBillingCron");

// src/queue-processor.ts
var RATE_LIMIT_PER_HOUR = 200;
var MAX_RETRIES = 3;
var BATCH_SIZE_PER_BRAND = 3;
async function handleQueueCron(env) {
  const supabase = createClient(env.SUPABASE_URL, env.SUPABASE_SERVICE_KEY);
  const { data: pendingBrands } = await supabase.from("share2dm_dm_queue").select("brand_id").eq("status", "pending").limit(100);
...
__name(handleQueueCron, "handleQueueCron");
async function processBrandQueue(supabase, brandId) {
  const oneHourAgo = new Date(Date.now() - 60 * 60 * 1e3).toISOString();
  const { count: recentSentCount } = await supabase.from("share2dm_dm_logs").select("*", { count: "exact", head: true }).eq("brand_id", brandId).gte("dm_sent_at", oneHourAgo);
  ...
  const { data: items } = await supabase.from("share2dm_dm_queue").select("*").eq("brand_id", brandId).eq("status", "pending").order("created_at", { ascending: true }).limit(batchSize);
  if (!items?.length) return;
  console.log(`[Queue] Brand ${brandId}: processing ${items.length} items (${sentInLastHour}/${RATE_LIMIT_PER_HOUR} sent in last hr)`);
  for (const item of items) {
    await supabase.from("share2dm_dm_queue").update({ status: "sending" }).eq("id", item.id);
    try {
      const response = await fetch(
        "https://graph.facebook.com/v21.0/me/messages",
        {
      ...
      if (response.ok) {
        await supabase.from("share2dm_dm_queue").update({ status: "sent", sent_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", item.id);
        await supabase.from("share2dm_dm_logs").upsert({
          id: crypto.randomUUID(),
        ...
        await supabase.from("share2dm_dm_queue").update({ status: "pending", error_message: `429 rate limited: ${resultText}` }).eq("id", item.id);
        console.warn(`[Queue] Brand ${brandId}: 429 rate limited, stopping batch`);
        return;
      } else {
        const newRetryCount = (item.retry_count ?? 0) + 1;
        const newStatus = newRetryCount >= MAX_RETRIES ? "failed" : "pending";
        await supabase.from("share2dm_dm_queue").update({
          status: newStatus,
...
__name(processBrandQueue, "processBrandQueue");
async function handleQueueStatus(url, env) {
  ...
  for (const status of statuses) {
    let query = supabase.from("share2dm_dm_queue").select("*", { count: "exact", head: true }).eq("brand_id", brandId).eq("status", status);
    if (campaignId) {
...
async function handleAdminStats(env) {
  ...
  const supabase = createClient(env.SUPABASE_URL, env.SUPABASE_SERVICE_KEY);
  const { count: totalBrands } = await supabase.from("share2dm_brands").select("*", { count: "exact", head: true });
  const { count: totalDMs } = await supabase.from("share2dm_dm_logs").select("*", { count: "exact", head: true });
  const todayStart = /* @__PURE__ */ new Date();
  todayStart.setUTCHours(0, 0, 0, 0);
  const { count: todayDMs } = await supabase.from("share2dm_dm_logs").select("*", { count: "exact", head: true }).gte("dm_sent_at", todayStart.toISOString());
  const { count: totalClicks } = await supabase.from("share2dm_dm_logs").select("*", { count: "exact", head: true }).not("link_clicked_at", "is", null);
  const queueStatuses = ["pending", "sending", "sent", "failed"];
  const queueCounts = {};
  for (const status of queueStatuses) {
    const { count } = await supabase.from("share2dm_dm_queue").select("*", { count: "exact", head: true }).eq("status", status);
    queueCounts[status] = count ?? 0;
  }
  const { data: brands } = await supabase.from("share2dm_brands").select("id, brand_name, ig_account_id, plan, created_at").order("created_at", { ascending: false });
  const startOfMonth = /* @__PURE__ */ new Date();
  startOfMonth.setUTCDate(1);
  startOfMonth.setUTCHours(0, 0, 0, 0);
  const brandStats = [];
  for (const brand of brands ?? []) {
    const { count: monthlyDMs } = await supabase.from("share2dm_dm_logs").select("*", { count: "exact", head: true }).eq("brand_id", brand.id).gte("dm_sent_at", startOfMonth.toISOString());
    const { count: monthlyClicks } = await supabase.from("share2dm_dm_logs").select("*", { count: "exact", head: true }).eq("brand_id", brand.id).gte("dm_sent_at", startOfMonth.toISOString()).not("link_clicked_at", "is", null);
    const { count: queuePending } = await supabase.from("share2dm_dm_queue").select("*", { count: "exact", head: true }).eq("brand_id", brand.id).eq("status", "pending");
    const { count: queueFailed } = await supabase.from("share2dm_dm_queue").select("*", { count: "exact", head: true }).eq("brand_id", brand.id).eq("status", "failed");
    brandStats.push({
      ...brand,
...
// src/index.ts
async function handleDebugSubscriptions(env) {
  const supabase = createClient(env.SUPABASE_URL, env.SUPABASE_SERVICE_KEY);
...
__name(handleDebugSubscriptions, "handleDebugSubscriptions");
async function handleMediaId(url, env) {
  const headers = {
    "Access-Control-Allow-Origin": "*",
    "Access-Control-Allow-Methods": "GET",
    "Content-Type": "application/json"
  ...
  try {
    const supabase = createClient(env.SUPABASE_URL, env.SUPABASE_SERVICE_KEY);
    const { data: brand } = await supabase.from("share2dm_brands").select("ig_account_id, ig_access_token").eq("id", brandId).single();
    if (!brand) {
      return new Response(JSON.stringify({ error: "brand not found" }), { status: 404, headers });
    }
    let mediaId = null;
    let nextUrl = `https://graph.facebook.com/v21.0/${brand.ig_account_id}/media?fields=id,shortcode&limit=50&access_token=${brand.ig_access_token}`;
    while (nextUrl && !mediaId) {
      ...
      const match = data.data?.find((m) => m.shortcode === shortcode);
      if (match) {
        mediaId = match.id;
      } else {
...
__name(handleMediaId, "handleMediaId");
var index_default = {
