...
const ClozetConnect = () => {
  ...
  return (
    <div className="min-h-screen pt-32 pb-24 px-6 relative bg-[#0B0914] flex flex-col items-center">
      {/* Background glow */}
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[1000px] h-[500px] bg-gradient-to-r from-pink-500/20 to-purple-500/20 blur-[150px] rounded-full pointer-events-none" />

      {/* Header */}
      <div className="text-center relative z-10 mb-20">
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-gradient-to-r from-pink-500/20 to-purple-500/20 border border-pink-500/30 mb-6">
          <Sparkles className="w-4 h-4 text-pink-400" />
          <span className="text-sm font-semibold text-pink-300">Clozet Connect</span>
        </div>
        ...
        <p className="text-sm text-gray-500">
          Powered by{' '}
          <a href="https://clozet.my" target="_blank" rel="noopener noreferrer" className="text-purple-400 hover:text-purple-300 transition-colors font-medium">
            Clozet
          </a>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {steps.map((step, i) => (
            <div key={i} className="relative group">
              ...
              {i < steps.length - 1 && (
                <div className="hidden md:block absolute top-10 left-[calc(50%+40px)] w-[calc(100%-80px)] h-px bg-gradient-to-r from-white/20 to-white/5 translate-x-[40px]" />
              )}
              <div className="flex flex-col items-center text-center p-8 rounded-3xl bg-white/[0.02] border border-white/5 hover:border-white/10 transition-all">
                <div className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${step.color} flex items-center justify-center text-white mb-6 shadow-lg group-hover:scale-105 transition-transform`}>
                  <span className="text-xs font-bold absolute -top-2 -right-2 w-6 h-6 rounded-full bg-white/10 border border-white/20 flex items-center justify-center text-white/70">{i + 1}</span>
                  {step.icon}
                </div>
          ...
          <div className="flex-1 space-y-8">
            ...
            <div className="space-y-4">
              <div className="flex items-start gap-4 p-4 rounded-2xl bg-white/[0.03] border border-white/5">
                ...
                <div>
                  <p className="text-white font-medium text-sm mb-1">{t('끊김 없는 재생', 'Seamless Playback')}</p>
              ...
              <div className="flex items-start gap-4 p-4 rounded-2xl bg-white/[0.03] border border-white/5">
                ...
                <div>
                  <p className="text-white font-medium text-sm mb-1">{t('네이티브 쇼핑 경험', 'Native Shopping Experience')}</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="p-8 rounded-3xl bg-[#151221]/50 border border-white/5 backdrop-blur-sm flex flex-col">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h3 className="text-xl font-bold text-white">{t('Clozet 스토어', 'Clozet Store')}</h3>
                <span className="text-xs text-green-400 font-medium">{t('모든 플랜', 'All Plans')}</span>
              </div>
            </div>
            <ul className="space-y-3 flex-1 mb-6">
              <li className="flex items-start gap-3">
                <div className="w-1.5 h-1.5 rounded-full bg-purple-400 mt-2 shrink-0" />
                <span className="text-gray-300 text-sm">{t('가입 즉시 사용 가능', 'Available immediately after sign-up')}</span>
              </li>
              <li className="flex items-start gap-3">
                <div className="w-1.5 h-1.5 rounded-full bg-purple-400 mt-2 shrink-0" />
                <span className="text-gray-300 text-sm">{t('숏폼에 최적화된 네이티브 구매 경험', 'Native purchase experience optimized for short-form')}</span>
              </li>
              <li className="flex items-start gap-3">
                <div className="w-1.5 h-1.5 rounded-full bg-purple-400 mt-2 shrink-0" />
                <span className="text-gray-300 text-sm">{t('별도 쇼핑몰 없이도 판매 시작', 'Start selling without your own store')}</span>
              </li>
            </ul>
            <button
          ...
          <div className="p-8 rounded-3xl bg-gradient-to-b from-purple-900/30 to-[#151221] border border-purple-500/30 flex flex-col relative overflow-hidden shadow-[0_0_40px_rgba(147,51,234,0.1)]">
            ...
            <ul className="space-y-3 flex-1 mb-6">
              <li className="flex items-start gap-3">
                ...
                <span className="text-gray-300 text-sm">{t('브랜드 자체 쇼핑몰로 직접 연결', 'Connect directly to your own shopping mall')}</span>
              </li>
              <li className="flex items-start gap-3">
                <div className="w-1.5 h-1.5 rounded-full bg-pink-400 mt-2 shrink-0" />
                <span className="text-gray-300 text-sm">{t('자사 브랜딩 & 고객 데이터 100% 유지', 'Keep 100% of your branding & customer data')}</span>
              </li>
              <li className="flex items-start gap-3">
                <div className="w-1.5 h-1.5 rounded-full bg-pink-400 mt-2 shrink-0" />
                <span className="text-gray-300 text-sm">{t('기존 결제·CS 시스템 그대로 활용', 'Leverage your existing payment & CS systems')}</span>
              </li>
            </ul>
            <button
